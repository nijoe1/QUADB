...
>(({ className, children, showCloseButton, ...props }, ref) => (
  <DialogPortal>
    ...
    >
      {children}
      {showCloseButton && (
        <DialogPrimitive.Close className="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-white transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-black focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-grey-100 data-[state=open]:text-grey-900">
          <X className="h-4 w-4" />
...
const DialogHeader = ({
  ...
  return (
    <div
      className={cn("flex flex-col space-y-1.5", position, className)}
      {...props}
    />
  );
};
