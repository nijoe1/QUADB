...
export const deployTestContracts = async (): Promise<ITestContracts> => {
  ...
  const MACIQVStrategy = await MACIQVStrategyFactory.deploy(Allo, "MACIQV");
  ...
  let types = [
    "((bool,bool,uint256,uint256,uint256,uint256),(address,(uint256,uint256),address,address,uint8,bytes,uint256))",
  ];
  ...
  const MACIQV_STRATEGY = await ethers.getContractAt("MACIQV", poolAddress);

  const maci = await MACIQV_STRATEGY.maci();

  const pollContracts = await MACIQV_STRATEGY.pollContracts();
