...
task(
  ...
  try {
    const MACIQVStrategy = await contractStates.getMACIQVStrategy();

    const isFinalized = await MACIQVStrategy.isFinalized();

    if (!isFinalized) {
      throw new Error("You cannot emergentWithdraw a non finalized round");
    }

    const isCancelled = await MACIQVStrategy.isCancelled();

    if (isCancelled) {
      throw new Error("You cannot emergentWithdraw a canceled round");
    }

    const finalizedAt = await MACIQVStrategy.finalizedAt();
    ...
    if (
      ...
      (await MACIQVStrategy.EMERGENCY_WITHDRAWAL_DELAY())
    ) {
      throw new Error(
    ...
    const token = await contractStates.getMACIQVToken();

    const emergencyWithdrawTx = await MACIQVStrategy.emergencyWithdraw(token);
