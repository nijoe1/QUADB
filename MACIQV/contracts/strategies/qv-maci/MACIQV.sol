...
// Core Contracts
import {MACIQVBase} from "./MACIQVBase.sol";

/// @notice This contract handles the quadratic funding mechanism using MACI (Minimal Anti-Collusion Infrastructure).
/// Allo x MACI Capital Constrained Quadratic Funding Strategy (MACIQV)
/// It extends the MACIQVBase contract and integrates MACI-related functionalities.
/// Inspired by CLR.Fund https://github.com/clrfund/monorepo/blob/develop/contracts/contracts/FundingRound.sol
contract MACIQV is MACIQVBase, DomainObjs, Params {
    /// ======================
    /// ======= Structs ======
    /// ======================
    ...
    /// @notice Initializes the MACIQV contract
    /// @param _allo The address of the Allo contract
    /// @param _name The name of the strategy
    constructor(address _allo, string memory _name) MACIQVBase(_allo, _name) {}
    ...
    function initialize(uint256 _poolId, bytes memory _data) external virtual override onlyAllo {
        ...
        _MACIQVStrategy_init(_poolId, _initializeParams);
    ...
    function _MACIQVStrategy_init(uint256 _poolId, InitializeParamsMACI memory _params) internal {
        _MACIQVBaseStrategy_init(_poolId, _params.initializeParams);
        ...
        voiceCredits = _params.maciParams.voiceCredits;
    ...
    function _allocate(bytes memory _data, address _sender) internal override onlyActiveAllocation {
        (PubKey memory pubKey, bytes memory _proof) =
            abi.decode(_data, (PubKey, bytes));
        ...
        emit Allocated(address(0), 0, token, _sender);
    }
    ...
    function _distributeFunds(bytes memory _claim) internal {
        ...
        uint256 amount = _getAllocatedAmount(recipient.totalVotesReceived);
        // uint256 amount = _getAllocatedAmount(claim.spent);
