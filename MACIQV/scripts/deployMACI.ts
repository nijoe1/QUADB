...
async function main() {
  ...
  if (!MACIDeployments?.MACIQVStrategy?.MACIQVStrategyAddress) {
    const MACIQVStrategyFactory = await ethers.getContractFactory("MACIQV");
    ...
    console.log("Deploying MACIQVStrategy...");
    console.log("Allo address:", Allo);

    const MACIQVStrategy = await MACIQVStrategyFactory.deploy(
      Allo,
      "MACIQVStrategyv1.0"
    );

    const MACIQVStrategyAddress = await MACIQVStrategy.getAddress();

    console.log("MACIQVStrategy deployed at:", MACIQVStrategyAddress);

    DeployedContracts.MACIQVStrategy = {
      name: "MACIQVStrategyv1.0",
      MACIQVStrategyAddress: MACIQVStrategyAddress,
    };
    deployments.write(DeployedContracts);

    // await verifyContract(MACIQVStrategyAddress, [Allo, "MACIQV"]);
  } else {
    console.log(
      "Reusing MACIQVStrategy:",
      MACIDeployments.MACIQVStrategy.MACIQVStrategyAddress
    );

    DeployedContracts.MACIQVStrategy = MACIDeployments.MACIQVStrategy;
  }
